...
function filterTypeLinks() {
  ...
  for (var i = 0; i < links.length; i++) {
    ...
    var linkTitle = links[i].children[0].children[0].textContent.split(' ')[1];
    var linkURL = links[i].children[0].children[1].textContent.split(' ')[1];
    var match = linkTitle.toUpperCase().indexOf(filter) > -1 || linkURL.toUpperCase().indexOf(filter) > -1;
...
function filterReadLinks() {
  ...
  for (var i = 0; i < links.length; i++) {
    var readStatus = links[i].children[0].children[2].textContent.split(' ')[1];
    var match = readStatus.toUpperCase().indexOf('TRUE') > -1;
    links[i].style.display = match ? "" : "none";
  }
}
...
function filterUnreadLinks() {
  ...
  for (var i = 0; i < links.length; i++) {
    var unreadStatus = links[i].children[0].children[2].textContent.split(' ')[1];
    var match = unreadStatus.toUpperCase().indexOf('FALSE') > -1;
    links[i].style.display = match ? "" : "none";
  }
};
"use strict";
...
function Link(link) {
  ...
  Link.prototype.createLink = function () {
    $.ajax({
      type: "POST",
      url: "https://url-lockbox-bs.herokuapp.com/api/v1/links",
      dataType: "json",
      data: { link: {
          title: this.title,
          url: this.url
      ...
      success: function success(linkObject) {
        var link = new Link(linkObject);
...
function markAsRead(e) {
  ...
  $.ajax({
    type: "PATCH",
    url: 'https://hot-reads-bs.herokuapp.com/api/v1/links/' + linkId,
    data: {
      link: { read: readStatus }
    }
...
function postHotReads(link) {
  ...
  }).done(updateHotReads).fail(displayFailure);
}
