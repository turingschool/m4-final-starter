...
function addEditForm(url, title, parent) {
  ...
  $(parent).prepend('URL: <input type="text" name="URL" id="link_url" value="' + url + '" /><br>\n    Title: <input type="text" name="Title" value="' + title + '" id="link_title" /><br>');
}
...
function makeButtons(index) {
  var linkStatus = $(this).find(".read-status").text();
...
$('#new-link-form').submit(function (e) {
  e.preventDefault;
  $('#ajax-errors').empty();
  var url = $('#new-link-form #link_url').val();
  var title = $('#new-link-form #link_title').val();
  $.ajax({
    type: "POST",
    url: '/api/v1/links',
    data: { url: url, title: title },
